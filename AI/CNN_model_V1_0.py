...
def trainning_process(path):
    ...
    y_train=label
    #ben
    train_model_v1_0(x_train,y_train)


def train_model_v1_0(save_model_path,x_train,y_train):
    ...
    regularizer = tf.truncated_normal_initializer(stddev=0.1) 
    ...
    for epoch in range(n_epoch):
        ...
        if np.sum(train_acc)/ n_batch > 0.998:
            break
        '''
        #validation
...
def prediction(start, end):
    ...
    with tf.Session() as sess:
        ...
        read_test_img(sample_name,data, start, end)    
        #data = test_data
        ...
        #打印出预测矩阵每一行最大值的索引
        print(tf.nn.softmax(classification_result,1).eval())
        #根据索引通过字典对应花的分类
        output = []
        output = tf.nn.softmax(classification_result,1).eval()
        row_excel = 1
        for i in range(len(output)):
            print("第",i+1,"个: "+str(output[i][1]))
            for cx in range(ws.min_row ,ws.max_row):
                if ws.cell(row=cx, column=1).value == None or ws.cell(row=cx, column=2).value == None:
                    continue
                small_image_name =  str(ws.cell(row=cx, column=1).value +'/' + ws.cell(row=cx, column=2).value) 
                if (small_image_name == str(sample_name[i])):
                    print ("sample_name[",str(cx),"]: ",str(sample_name[i])," output = ", str(output[i][1]))
                    ws.cell(row=cx, column=4).value = str(output[i][1])
...
#temp for number111 in range(1,60):
#temp    prediction((number111 - 1) * 200, number111 * 200)
