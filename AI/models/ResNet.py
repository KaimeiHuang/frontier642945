...
def Conv2d_BN(x, nb_filter, kernel_size, strides=(1, 1), padding='same', activation='relu', name=None):
    if name is not None:
        bn_name = name + '_bn'
    ...
    x = keras.layers.Conv2D(nb_filter, kernel_size, padding=padding, strides=strides, activation=activation, name=conv_name)(x)
...
def identity_Block(inpt, nb_filter, kernel_size, activation='relu', strides=(1, 1), with_conv_shortcut=False):
    x = Conv2d_BN(inpt, nb_filter=nb_filter, kernel_size=kernel_size, strides=strides, padding='same', activation=activation)
    x = Conv2d_BN(x, nb_filter=nb_filter, kernel_size=kernel_size, padding='same', activation=activation)
    if with_conv_shortcut:
        shortcut = Conv2d_BN(inpt, nb_filter=nb_filter, strides=strides, kernel_size=kernel_size, activation=activation)
...
def resnet34(image_sizes_list,classes,parameters_list,activation_function, init_mode):
    ...
    #conv1
    x = Conv2d_BN(x, nb_filter=64, kernel_size=(7, 7), strides=(2, 2), padding='valid', activation=activation_function)
    ...
    x = keras.layers.Dense(classes, activation=activation_function, kernel_initializer=init_mode)(x)
    ...
    #model = ResNet50(include_top=False, weights="imagenet", input_shape=(width, height, channel))
    model = ResNet50(include_top=False, input_shape=(width, height, channel))
    # Get the ResNet50 layers up to res5c_branch2c
    x = model(inputs)
    x = tf.keras.layers.Dense(2, activation='sigmoid',
                              name='prediction')(x)

    model = tf.keras.models.Model(inputs, x, name='model')
