...
def prepare_data(images_dir_root, size_square, image_suffix):
    ...
    img_label = []
    pos = 0
    print("images_dir_root in prepae_data:",images_dir_root)
    image_dir_list = glob(join(images_dir_root, r'*/'))
    for image_dir in image_dir_list :
        print("image_dir:",image_dir)
        image_png_address = glob(join(image_dir, '*.png'))
        if  image_png_address!= []:
            print("exist_cut mage_dir:",image_dir)
            continue
        print(" need_cut image_dir:",image_dir)
        ...
        for index_xml in range(len(xml_files)):
            ...
            if os.path.exists(xml_files[index_xml].split('xml')[0] + image_suffix):
                image_address = xml_files[index_xml].split('xml')[0] + image_suffix
            else:
                continue

            print("image_address:",image_address)
            slide = openslide.open_slide(image_address)
            image_large = get_ret_ifo(xy_list,slide,image_address,size_square,size_square,3,0.3)
...
if __name__ == '__main__':
    ...
    if(args.prepare_types=="1"):
        print("args.prepare_types:",args.prepare_types)
