...
def get_ret_ifo(xy_list:list,slide,svs_address,window_size:int,stride:int,Points_Con_Thre:int,Area_Ratio_Thre:float):
    ...
    for i in range(len(xy_list)):
        ...
        for points in xy_list[i]:
            if i==0:
                luncancer += 1
                print("   Deal with {0}th Cancer area....".format(luncancer))
            if i==1:
                health += 1
            ...
            img_region = slide_window(slide,svs_address, x,y,w,h,window_size, stride,i,luncancer,health,i,contours,Points_Con_Thre,Area_Ratio_Thre)
